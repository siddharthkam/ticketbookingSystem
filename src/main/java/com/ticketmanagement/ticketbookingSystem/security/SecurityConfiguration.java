...
public class SecurityConfiguration extends WebSecurityConfigurerAdapter {
	...
	protected void configure(HttpSecurity http) throws Exception {
		...
		http.authorizeRequests().antMatchers("/user/ticketbyid/**").permitAll();
	}
	@Autowired
	public void globalConfiguration(AuthenticationManagerBuilder auth) throws Exception{
		...
		auth.inMemoryAuthentication().withUser("SIDDHARTH19").password("{noop}PASSWORD").roles("USER","ADMIN");
		...
		auth.inMemoryAuthentication().withUser("SIDDHARTH12").password("{noop}PASSWORD").roles("USER","ADMIN");
